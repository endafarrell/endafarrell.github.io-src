...
class HtmlTreeNode(object):
    ...
    def __str__(self):
        ...
        if self.parent or self.include_title:
            if ("'" in self.id or "'" in self.header) and '"' not in self.id:
                ret = f"""<a class="toc-href"" href="#{self.id}" title="{self.header}">{self.header}</a>"""
            else:
        ...
        if self.children:
            ret += "<ul class='flex-grid'>{}</ul>".format("{}" * len(self.children)).format(
                *self.children
            )

        # each list
        if self.parent or self.include_title:
            ret = "<li class='col'>{}</li>".format(ret)

        # end wrapper
        if not self.parent:
            if self.include_title:
                ret = "<div id='toc'><ul class='flex-grid'>{}</ul></div>".format(ret)
            else:
